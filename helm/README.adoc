...
tls:
  enabled: true
  serverCertificatesSecret: 'tls-cert'
...
To access non-http - messaging - endpoints through an OpenShift route we need to enable TLS, see https://medium.com/itnext/how-to-access-your-app-behind-an-openshift-router-87cbae3e7185[details explained here]. Alternatively you can use _LoadBalancer_ (in cloud) or _NodePort_ services. 
So you need to create the `tls-cert` secret in the namespace with the key/certificate as below.
...
Open https://semp-pubsubplus-helm.apps.your_cluster_domain.com and login with `admin` and the password set in the values file. 
