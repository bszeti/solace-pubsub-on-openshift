...
| `SOLACE`` (SMF JMS)
| `tcp://host:55555` tcps://host:55443
| `SOLACE` (over websocket)
| `ws://host:8008` wss://host:1443
| `AMQP` (Qpid JMS)
| `amqp://host:5672` `amqps://host:5671`
| `MQTT` (Paho Java)
| `tcp://host:1883` `ssl://host:8883`
| `MQTT` (over websocket)
| `ws://host:8000` `wss://host:8443`
|===

[NOTE]
The usual protocol specific parameters are accepted in the URL, e.g. `failover:(amqps://host1:5671,amqps://host2:5671)?jms.prefetchPolicy.all=10&failover.randomize=true&failover.nested.transport.trustStoreLocation=./client.ts&failover.nested.transport.trustStorePassword=changeit`
...
Check metric query `solace_queue_spool_usage_msgs{queue_name='q1'}` to see current length size or `solace_queue_msg_spooled{queue_name='q1'}` for total messages sent to the queue.
